    ...
    <div className={`markdown-content ${className}`}>
      <ReactMarkdown
        ...
        components={{
          ...
          h3: ({ node, ...props }) => (
            <h3 className="text-base font-semibold text-gray-700 mb-2 mt-3" {...props} />
          ),
          p: ({ node, ...props }) => (
            <p className="text-gray-700 mb-3 leading-relaxed" {...props} />
          ),
          ul: ({ node, ...props }) => (
            <ul className="list-disc list-inside space-y-1 mb-3 text-gray-700 pl-2" {...props} />
          ),
          ol: ({ node, ...props }) => (
            <ol className="list-decimal list-inside space-y-1 mb-3 text-gray-700 pl-2" {...props} />
          ),
          li: ({ node, ...props }) => (
          ...
          code: ({ node, inline, ...props }) =>
            inline ? (
              ...
              <pre className="bg-gray-900 text-green-400 p-4 rounded-lg overflow-x-auto mb-3 text-sm font-mono">
                <code {...props} />
              </pre>
            ),
          blockquote: ({ node, ...props }) => (
            <blockquote
              className="border-l-4 border-blue-300 pl-4 italic text-gray-600 mb-3"
              {...props}
            />
          ),
